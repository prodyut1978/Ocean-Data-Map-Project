#!/usr/bin/env bash

# Usage: ./runserver.sh <port> <optional_dataset_config_file.json>
# <optional_dataset_config_file.json>:  Specify a non-default dataset config file to load the Navigator with.
#                                       Argument not required.
WORKER_THREADS=1

gunicorn --error-log ${HOME}/gunicorn.log -w $(nproc) --threads $((WORKER_THREADS)) --worker-class=gthread -t 300 --graceful-timeout 300 --preload -b 0.0.0.0:5000 "oceannavigator:create_app()" $2 --daemon
